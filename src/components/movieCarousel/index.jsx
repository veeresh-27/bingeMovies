...
const MovieCarousel = ({ content, type }) => {
  ...
  const items = content?.map((incontent) => (
    <div
      ...
      onClick={() =>
        navigate(
          `/movieinfo/${incontent?.media_type ? incontent?.media_type : type}/${
            incontent?.id
          }`
        )
      }
    >
      <div className="carousel__imageContainer">
        <img
          src={
            incontent.poster_path
              ? `${img_300}/${incontent.poster_path}`
              : noPicture
          }
          alt={incontent?.name}
      ...
      <p
        title={incontent?.original_title || incontent?.name}
